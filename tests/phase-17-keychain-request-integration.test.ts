...
after(async () => {
  ...
  try {
    await client.query("delete from rate_limit_attempts where scope in ('custom-request-attachment-upload', 'keychain-price-estimate') and limiter_key = $1", [createHash('sha256').update(TEST_IP).digest('hex')]);
...
test('public estimate route accepts valid measurements, rejects invalid input, and never leaks raw config', { skip: !canRun }, async () => {
  const valid = await fetch(`${BASE_URL}/api/public/keychain-price-estimate`, {
  ...
  const invalid = await fetch(`${BASE_URL}/api/public/keychain-price-estimate`, {
    method: 'POST', headers: { ...HEADERS, 'content-type': 'application/json' },
