...
import { calculateKeychainPriceRange } from '../src/lib/pricing/keychain-price-range';
...
test('valid keychain measurements produce a rounded advisory range', () => {
  assert.deepEqual(calculateKeychainPriceRange({ weightGrams: 10, printMinutes: 30, unitCostVndPerGram: 160, config }), {
    minPriceVnd: 15_000,
    maxPriceVnd: 21_000,
  });
});
...
test('price-estimate route validates strict reasonable bounds and returns an allowlisted DTO', async () => {
  const source = await readFile('src/app/api/public/keychain-price-estimate/route.ts', 'utf8');
  assert.match(source, /\.positive\(\)\.max\(2_000\)/);
  assert.match(source, /\.positive\(\)\.max\(7 \* 24 \* 60\)/);
  const responseKeys = Object.keys(calculateKeychainPriceRange({ weightGrams: 10, printMinutes: 30, unitCostVndPerGram: 160, config }));
