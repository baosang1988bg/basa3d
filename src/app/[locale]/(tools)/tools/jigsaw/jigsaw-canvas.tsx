...
export default function JigsawCanvas({ group }: { group: THREE.Group }) {
  ...
  useEffect(() => {
    ...
    camera.position.set(center.x + size * 0.7, center.y + size * 0.9, center.z + size * 0.9);
    ...
    renderer.domElement.dataset.testid = 'jigsaw-preview-canvas';
  ...
  return <div className="relative h-full min-h-[300px]" ref={hostRef} aria-label="Bản xem trước mảnh ghép 3D">{error ? <p role="alert">{error}</p> : null}</div>;
}
