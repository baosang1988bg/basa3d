...
export default function HingeBoxCanvas({ group }: { group: THREE.Group }) {
  ...
  useEffect(() => {
    ...
    camera.position.set(center.x, center.y + size * 0.9, center.z + size * 0.9);
    scene.add(group, new THREE.HemisphereLight(0xffffff, 0x555555, 2));
    const light = new THREE.DirectionalLight(0xffffff, 3); light.position.set(100, 300, 200); scene.add(light);
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    renderer.domElement.dataset.testid = 'hinge-box-preview-canvas';
  ...
  return <div className="relative h-full min-h-[300px]" ref={hostRef} aria-label="Bản xem trước hộp bản lề 3D (layout phẳng để in)">{error ? <p role="alert">{error}</p> : null}</div>;
}
