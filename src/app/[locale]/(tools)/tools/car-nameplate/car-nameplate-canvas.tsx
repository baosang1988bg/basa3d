...
export default function CarNameplateCanvas({ group }: { group: THREE.Group }) {
  ...
  useEffect(() => {
    ...
    const light = new THREE.DirectionalLight(0xffffff, 3); light.position.set(100, 200, 200); scene.add(light);
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    renderer.domElement.dataset.testid = 'car-nameplate-preview-canvas';
  ...
  return <div className="relative h-full min-h-[300px]" ref={hostRef} aria-label="Bản xem trước bảng tên xe 3D">{error ? <p role="alert">{error}</p> : null}</div>;
}
