...
export default function DualTextCanvas({ group }: { group: THREE.Group }) {
  ...
  useEffect(() => {
    ...
    renderer.domElement.dataset.testid = 'dual-text-preview-canvas';
  ...
  return <div className="relative h-full min-h-[300px]" ref={hostRef} aria-label="Bản xem trước chữ ảo ảnh 3D">{error ? <p role="alert">{error}</p> : null}</div>;
}
