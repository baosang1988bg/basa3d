...
test('visitor generates a Dual Text STL, submits it, and admin can see the request', async ({ page }, testInfo) => {
  const phone = `08${Date.now().toString().slice(-8)}`;
  const customerName = `Dual Text E2E ${randomUUID().slice(0, 6)}`;
  await page.goto('/tools/dual-text');
  await page.getByLabel('Từ 1 (góc nhìn 1)', { exact: true }).fill('HI');
  await page.getByLabel('Từ 2 (góc nhìn 2)', { exact: true }).fill('OK');
  await page.getByRole('combobox', { name: 'Phông chữ', exact: true }).selectOption('Anton');
  await expect(page.locator('[data-testid="dual-text-preview-canvas"]')).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('dual-text-preview.png') });
  for (const label of ['Tải STL', 'Tải 3MF (có màu)']) {
  ...
  await expect(page.locator('#description')).toHaveValue(/Chữ ảo ảnh 2 góc nhìn: "HI" \/ "OK"/);
  ...
  const email = `dual-text-admin-${randomUUID().slice(0, 8)}@example.com`; const password = 'e2e-test-password-123';
  ...
  try {
    ...
    try { await db.query("insert into staff_profiles (id, full_name, role, is_active) values ($1, 'Dual Text E2E Admin', 'OWNER', true)", [data.user.id]); } finally { await db.end(); }
