...
test('visitor generates a Hinge Box STL, submits it, and admin can see the request', async ({ page }, testInfo) => {
  const phone = `01${Date.now().toString().slice(-8)}`;
  const customerName = `Hinge Box E2E ${randomUUID().slice(0, 6)}`;
  await page.goto('/tools/hinge-box');
  await expect(page.locator('[data-testid="hinge-box-preview-canvas"]')).toBeVisible({ timeout: 15_000 });
  await page.locator('#hinge-cols').fill(String(2));
  await page.screenshot({ path: testInfo.outputPath('hinge-box-preview.png') });
  ...
  await expect(page.locator('#description')).toHaveValue(/Hộp có bản lề liền khối:/);
  ...
  const email = `hinge-box-admin-${randomUUID().slice(0, 8)}@example.com`; const password = 'e2e-test-password-123';
  ...
  try {
    ...
    try { await db.query("insert into staff_profiles (id, full_name, role, is_active) values ($1, 'Hinge Box E2E Admin', 'OWNER', true)", [data.user.id]); } finally { await db.end(); }
