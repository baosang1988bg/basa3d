...
test('visitor generates a Car nameplate STL, submits it, and admin can see the request', async ({ page }, testInfo) => {
  const phone = `09${Date.now().toString().slice(-8)}`;
  const customerName = `Car Nameplate E2E ${randomUUID().slice(0, 6)}`;
  await page.goto('/tools/car-nameplate');
  await page.getByLabel('Tên', { exact: true }).fill('LEO');
  await page.getByRole('combobox', { name: 'Phông chữ', exact: true }).selectOption('Anton');
  await expect(page.locator('[data-testid="car-nameplate-preview-canvas"]')).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('car-nameplate-preview.png') });
  ...
  await expect(page.locator('#description')).toHaveValue(/Bảng tên xe ảo ảnh \(Sedan\): "LEO"/);
  ...
  const email = `car-nameplate-admin-${randomUUID().slice(0, 8)}@example.com`; const password = 'e2e-test-password-123';
  ...
  try {
    ...
    try { await db.query("insert into staff_profiles (id, full_name, role, is_active) values ($1, 'Car Nameplate E2E Admin', 'OWNER', true)", [data.user.id]); } finally { await db.end(); }
